/**
 * public/javascripts/tabulacao.js
 * Client-side logic for managing "tabulação" views and exports.
 * Responsibilities:
 * - Load and cache tabulation data from the backend (/whatsapp/tabulacoes).
 * - Render filtered lists for different statuses (aniversariantes, sem-interesse, etc.).
 * - Support month filtering for aniversariantes and export to Excel.
 * - Provide real-time updates via WebSocket (chat-returned events).
 */
let tabulacoesCache = {};
let selectedMonth = null; // YYYY-MM
// Current selected status (controls which tab/list is shown)
let currentStatus = 'aniversariantes';
let wsWhatsappTabulacao = null;

// formatarStatus(status)
// - Human-friendly label mapping for internal status keys.
...
// - Converts ISO-like date string to a localized 'pt-BR' date string.
...
// getInitialStatus()
// - Reads the ?status= query param; defaults to 'aniversariantes'
...
// getInitialMonth()
// - Reads the ?mes=YYYY-MM query param to initialize month filter
...
// - Shows/hides the month filter UI when status === 'aniversariantes'
...
// obterItemsFiltrados(status)
// - Central filter used by render and export functions. Applies the selected
//   month filter when status === 'aniversariantes'. Returns an array of items.
...
// - Render the tabulated list for a given status. Updates UI state, toggles
//   filters, and uses cached data from `tabulacoesCache`.
...
// carregarTabulacoes()
// - Loads tabulations from the server and fills `tabulacoesCache`. On error
//   it empties the cache and renders an empty state.
...
// exportarExcel()
// - Exports the current tab view to XLSX. Deduplicates entries by chatId and
//   supports a special two-column export for aniversariantes (phone + date).
...
// - Connects to the atendimento WebSocket (ws-atendimento) and listens for
//   'chat-returned' events to remove returned contacts from the tabulation lists.
...
// returnToAtendimento(chatId)
// - POSTs to backend to return a tabulated contact to active atendimento and
//   refreshes local data on success.
